...
/* I'm running this query to get a feel for the data in the table I just created. */
SELECT * FROM CityBikeLanes
    LIMIT 10;
    
SELECT * FROM CityBikeLanes 
    WHERE street = 'Walnut';
/* There are two different rows displayed after running this query due to two seperate safety ratings. */

/* List all the bike lanes that have an average safety rating of 4.0 or higher.
Display the average safety rating for each lane, and a label that says "Safe Lane." */
...
/* Use a window function to display each bike lane's seperate safety ratings AND the average rating. */
SELECT street, safetyrating,
AVG(safetyrating) OVER (PARTITION BY street) as "Average_safety_rating"
FROM CityBikeLanes;

/* A window function is necesary for this query if you want BOTH safety ratings and the average between the two. */
SELECT street, safetyrating, AVG(safetyrating)
FROM CityBikeLanes
WHERE street='2nd'
GROUP BY street, safetyrating

/* Display the safety ratings and average rating for 2nd street. */
SELECT street, safetyrating,
AVG(safetyrating) OVER (PARTITION BY street) as "Average_safety_rating"
FROM CityBikeLanes
WHERE street='2nd';

/* List all bike lanes, both safety ratings for each lane, the average safety rating for each lane, and a label with the recommendation as "Remove," "Leave As-Is," or "Improvements Needed." */
SELECT street, safetyrating, AVG(safetyrating)
    OVER (PARTITION BY street) AS "Avg_safety_rating",
CASE
