...
/* What is the average/max/min worldwide sales for the movies? */
SELECT ROUND(AVG(worldwide)) from topmovies;
SELECT ROUND(MAX(worldwide)) from topmovies;
SELECT ROUND(MIN(worldwide)) from topmovies;

/* Use HAVING */
/* Select studios that made over 5000 worldwide sales. */
SELECT studio, SUM(worldwide) AS total_worldwide_sales FROM topmovies
    GROUP BY studio
    HAVING total_worldwide_sales > 5000;

/* Select studios that have an average of over 900 worldwide sales per movie. */
SELECT studio, AVG(worldwide) AS avg_worldwide_sales FROM topmovies
    GROUP BY studio
    HAVING avg_worldwide_sales > 900;
    

/* Use AND and OR */
/* Select movies that had more than 500 domestic sales and were made before 2010. */
SELECT title from topmovies
    WHERE domestic > 500
    AND year < 2010;
/* How many movies fit the above criteria? */
SELECT COUNT(*) from topmovies
    WHERE domestic > 500
    AND year < 2010;
/*Select movies that had less than 500 worldwide sales or were made by WB studio. */
SELECT * FROM topmovies
    WHERE worldwide < 500
    OR studio = "WB"
    ORDER BY studio;
/* How many movies that had less than 500 worldwide sales were made by WB studio? */
SELECT COUNT(*) from topmovies
    WHERE worldwide < 50
    AND studio = "WB";
    
    
/* Use CASE to create new column */
SELECT title, year, 
    CASE
        WHEN year >= 2020 THEN "post_COVID"
        WHEN year < 2020 THEN "pre_COVID"
    END as "COVID_timeline"
FROM topmovies;

SELECT title, worldwide, 
    CASE
        WHEN worldwide > 863 THEN "above avg"
        WHEN worldwide < 863 THEN "below avg"
            ELSE "avg"
    END as "avg_worldwide_sales_comparison"
FROM topmovies;

/* Use COUNT to quantify how many movies were above/below avg in worldwide sales. */
SELECT COUNT(*), 
    CASE
        WHEN worldwide > 863 THEN "above avg"
        WHEN worldwide < 863 THEN "below avg"
    ...
    END as "avg_worldwide_sales_comparison"
FROM topmovies
GROUP BY avg_worldwide_sales_comparison;
